    ...
    if graphics_mode == 'integrated':
        ...
        if logging.getLogger().level == logging.DEBUG:
            service = subprocess.run(
                ["systemctl", "disable", "nvidia-{hibernate,suspend,resume}"])
        else:
            service = subprocess.run(
                ["systemctl", "disable", "nvidia-{hibernate,suspend,resume}"],
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if service.returncode == 0:
            print('Successfully disabled nvidia suspend services')
        else:
            logging.error("An error ocurred while disabling services")
        ...
        if logging.getLogger().level == logging.DEBUG:
            ...
            service = subprocess.run(
                ...
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if service.returncode == 0:
            print('Successfully enabled nvidia suspend services')
        else:
            logging.error("An error ocurred while enabling services")
...
def rebuild_initramfs():
    ...
    # LunaOS with dracut
    elif os.path.exists('/etc/lunaos-release') and os.path.exists('/usr/bin/dracut'):
        command = ['dracut-rebuild']
    else:
        command = []
