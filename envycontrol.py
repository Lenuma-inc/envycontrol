...
options nvidia-current NVreg_UsePageAttributeTable=1 NVreg_PreserveVideoMemoryAllocations=1 NVreg_TemporaryFilePath=/var/tmp
'''
...
def graphics_mode_switcher(graphics_mode, user_display_manager, enable_force_comp, coolbits_value, rtd3_value, use_nvidia_current):
    ...
    if graphics_mode == 'integrated':
        ...
        if service.returncode == 0:
            print('Successfully disabled nvidia-{hibernate,suspend,resume}')
        else:
        ...
        if logging.getLogger().level == logging.DEBUG:
            ...
            service = subprocess.run(
                ...
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if service.returncode == 0:
            print('Successfully enabled nvidia-{hibernate,suspend,resume}')
        else:
            logging.error("An error ocurred while enabling service")
